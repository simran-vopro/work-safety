    ...
    return (
        <div className="container-padding">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {products.map((item) => (
                    <div key={item._id} className="flex flex-col cursor-pointer mb-5 group">
                        {/* Added group class here for hover states */}
                        <div onClick={() => {
                            navigate("/projectDetails", {
                                state: item
                            });
                        }} className="relative bg-[#f5f5f5] h-72 overflow-hidden flex items-center justify-center transition-transform duration-300 group-hover:scale-101">
                            <img
                                src={item["Image Ref"]}
                        ...
                        <p className="app-text h-auto mt-2">{item.Category1?.Category1 || "No Category"}</p>

                        <h3 className="text-sm text-gray-700 group-hover:text-orange-500 font-semibold mt-1">{item.ExtendedCharacterDesc}</h3>
                    </div>
                ))}
            </div>
        </div>
